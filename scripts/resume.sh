...
# --- Wait for an EC2 instance to register with ECS ---
echo ""
echo "=== Waiting for EC2 instance to register with ECS ==="
INSTANCE_ID=""
for i in $(seq 1 24); do
  ...
  echo "  Attempt $i/24: waiting for instance ..."
  sleep 10
done

if [[ -z "$INSTANCE_ID" ]]; then
  echo "WARNING: No instance registered after timeout — skipping restore. Run resume.sh again once instances are up."
  exit 1
fi

# --- Restore records.json from S3 ---
...
echo ""
echo "=== Restoring records.json from s3://$BUCKET/records.json ==="

CMD_ID=$(aws ssm send-command \
  --instance-ids "$INSTANCE_ID" \
  --document-name AWS-RunShellScript \
  --parameters "commands=[\"for i in \\$(seq 1 18); do mount | grep -q efs && break; echo Waiting for EFS mount...; sleep 10; done && mkdir -p /ecs/logs/data && aws s3 cp s3://${BUCKET}/records.json /ecs/logs/data/records.json && echo Restored || echo No backup found - starting fresh\"]" \
